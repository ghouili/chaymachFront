...
const RubricModal = ({ modalOpen, toggleModal, fetchData, data }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let url, result;
      if (data) {
        url = `${path}rubrics/${formValues.id}`;
    ...
    <div>
      ...
      {modalOpen && (
        ...
        >
          ...
          <div className="relative p-4 w-full max-w-2xl max-h-full overflow-y-auto overflow-x-hidden">
            {/* Modal content */}
            <div className="relative bg-white border rounded-lg shadow dark:bg-gray-700">
              ...
              <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                  {formValues.id
                    ? "Mettre a jour Ce Rubrique"
                    : "Ajouter Nouveau Rubrique"}
                </h3>
                <button
                  onClick={toggleModal}
                  type="button"
